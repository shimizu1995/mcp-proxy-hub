...
- **stdio型サーバー**:
  ...
  - `timeout`: 下流サーバーへのツール呼び出しのタイムアウト（ミリ秒、オプション。トップレベルの `timeout` を上書き。`0` でタイムアウト無効）
...
- **トップレベルの `timeout`**: 全サーバーに適用されるグローバルなデフォルト値（ミリ秒）
- **サーバーごとの `timeout`**: 該当サーバーについてグローバル値を上書き
- **`0`**: タイムアウトを無効化（上限なし）
- **未指定**: MCP SDK のデフォルト（60 秒）にフォールバック

`setTimeout` の最大安全値である `2147483647` ms を超える値は自動的にその上限に丸められます。負数・`NaN`・非数値は無効扱いで、警告を出して次のレベルにフォールスルーします。
